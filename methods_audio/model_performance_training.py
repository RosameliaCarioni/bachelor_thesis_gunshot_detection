...
def train_performance_k_fold(
    ...
    for train, test in kfold.split(x, y):
        ...
        print(
            "%s: %.2f%%" % (model.metrics_names[1], hist.history["val_accuracy"][epoch - 1] * 100)
        )  # TODO: check if it should be epoch-1 or -1 only
...
def plot_confusion_matrix(confusion_matrices, destination_file: str, title_plot: str):
    ...
    sns.heatmap(mean_matrix, xticklabels=label_names, yticklabels=label_names, annot=True, fmt="g")
